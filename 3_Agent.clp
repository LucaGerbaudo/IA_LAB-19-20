...
(defrule inerzia10
	(status (step ?s)(currently running))
		(not (exec  (action fire) (x 8) (y 9)))
=>


	(assert (exec (step ?s) (action fire) (x 8) (y 9)))
     (pop-focus)
)

(defrule inerzia11
	(status (step ?s)(currently running))
		(not (exec  (action fire) (x 7) (y 7)))
=>


	(assert (exec (step ?s) (action fire) (x 7) (y 7)))
     (pop-focus)
)
...
(defrule inerzia20
	(status (step ?s)(currently running))
	(not (exec  (action guess) (x 9) (y 9)))
=>


	(assert (exec (step ?s) (action guess) (x 9) (y 9)))
     (pop-focus)
...
(defrule inerzia21
	(status (step ?s)(currently running))
	(not (exec  (action guess) (x 3) (y 2)))
=>


	(assert (exec (step ?s) (action guess) (x 3) (y 2)))
     (pop-focus)
