package probability.bayes.exact;
...
public class EliminationAsk implements BayesInference {
	...
	public CategoricalDistribution eliminationAsk(final RandomVariable[] X,
		...
		int widthOrdinamento = Integer.MIN_VALUE;
		...
		for (RandomVariable var : order(bn, VARS)) {
			// factors <- [MAKE-FACTOR(var, e) | factors]
			Factor _f = makeFactor(var, e, bn);
			factors.add(0, _f);
			if(_f.getArgumentVariables().size() > widthOrdinamento) widthOrdinamento =_f.getArgumentVariables().size();
		...
		if(product.getArgumentVariables().size() > widthOrdinamento) widthOrdinamento = product.getArgumentVariables().size();
		// Note: Want to ensure the order of the product matches the
		// query variables

		System.out.println("Complessità con width : O(2^" + widthOrdinamento + " * " + VARS.size()+ ")");

		return ((ProbabilityTable) product.pointwiseProductPOS(_identity, X)).normalize();
	}

	/**
	 * Variable Elimination, versione DARWICHE
	 */
	public CategoricalDistribution eliminationAsk(BayesianNetwork bn,
		...
		//Rimozione variabili di query
		pi.removeAll(Arrays.asList(Q));
		//Creazione dei fattori per le variabili
		for (RandomVariable var: bn.getVariablesInTopologicalOrder()) {
			Factor _f = makeFactor(var, e, bn);
			S.add(0, _f);
			if(_f.getArgumentVariables().size() > widthOrdinamento) widthOrdinamento =_f.getArgumentVariables().size();
		}
		//Scorrimento in ordine delle variabili
		for (int i = 0; i < pi.size(); i++) {
			List<Factor> toMultiply = new ArrayList<>();
			//Si prendono tutti i fattori in cui compare la variabile
			for (Factor fk: S) {
				if (fk.getArgumentVariables().contains(pi.get(i))) {
					toMultiply.add(fk);
				}
			}
			//Moltiplicazione e somma sui fattori della variabile
			if (toMultiply.size() > 0) {
				f = pointwiseProduct(toMultiply);
				Factor fi = ((ProbabilityTable)f.sumOut(pi.get(i))).normalize();

				S.removeAll(toMultiply);
				S.add(fi);

				if(fi.getArgumentVariables().size() > widthOrdinamento) widthOrdinamento =fi.getArgumentVariables().size();
			}
		}
		System.out.println("Complessità con width : O(2^" + widthOrdinamento + " * " + pi.size() + ")");
		return ((ProbabilityTable)pointwiseProduct(S)).normalize();
	}
