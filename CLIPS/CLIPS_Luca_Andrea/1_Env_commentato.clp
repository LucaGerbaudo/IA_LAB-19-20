...
; definizione di  CONTENUTO e STATO di una cella
(deftemplate cell
	(slot x)
	(slot y)
...
; definizione di nave orizzontale
...
; definizione del CONTENUTO di una cella 
(deftemplate k-cell 
	(slot x)
	(slot y)
...
; definizione dei numeri a fianco delle righe (num = tot celle con nave)
(deftemplate k-per-row
	(slot row)
	(slot num)
)
; definizione dei numeri a fianco delle colonne (num = tot celle con nave)
...
; FIRE: asserisce azione di fire in x y, decrementa fires e incrementa step
; fires = contatore di quanti colpi ancora da sparare ( max 5)
; step = contatore degli step fatti (max 100)
(defrule action-fire 
...
; GUESS: asserisce azione di guess su cella x y, decrementa  guesses e incrementa step
; guesses =  contatore di quante bandierine ancora da mettere (max 20)
(defrule action-guess
        ?us <- (status (step ?s) (currently running))
	(exec (step ?s) (action guess) (x ?x) (y ?y))
	?mvs <- (moves (guesses ?ng &:(> ?ng 0)))
=>
	;(printout t "EXEC GUESS on [" ?x "," ?y "]" crlf)
	;(facts)
	(assert (guess ?x ?y))
        (modify ?us (step (+ ?s 1)) )
...
; UNGUESS: retract su azione di guess fatta su cella x y, incrementa contatore guess e step
...
; SOLVE: asserisce azione di solve e cambia lo stato currently da running a stopped
...
; incrementa il contatore di azioni di fire andate a segno per il calcolo del punteggio
(defrule fire-ok
	(fire ?x ?y)
...
; incrementa il contatore di buchi nell'acqua per il calcolo del punteggio
(defrule fire-ko
	(fire ?x ?y)
...
; assert di barca orizzontale affondata per punteggio finale
...
; assert di barca verticale affondata per punteggio finale
...
; conta dei guessed azzeccati in caso di solve
...
=>
	;(printout t crlf)
	;(printout t "Guess [" ?x "," ?y "]: OK!!!")
	(modify ?st (num_guess_ok (+ 1 ?gok)))
	(modify ?c (status guessed))
)

; conta dei guessed sbagliati in caso di solve
...
; conta delle barche non colpite in caso di solve 
...
; conta delle barche non affondate in caso di solve
